...
// ─── Avatar del autor ─────────────────────────────────────────────────────────
function AutorAvatar({ nombre, avatarColor, rol, autorId }) {
  const t = useT();
  const navigate = useNavigate();
  const isCreador = rol === 'creador' || rol === 'moderador';

  return (
    <div
      className={`flex items-center gap-2.5 shrink-0 ${isCreador ? 'cursor-pointer hover:opacity-80 transition-opacity' : 'cursor-default'}`}
      onClick={isCreador && autorId ? () => navigate(`/creator/${autorId}/perfil`) : undefined}
      title={isCreador ? `Ver perfil de ${nombre}` : undefined}
    >
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-5 space-y-4 shadow-sm hover:shadow-md transition-all">
      ...
      <div className="flex items-start justify-between gap-3">
        <AutorAvatar nombre={post.autor_nombre} avatarColor={post.autor_color} rol={post.autor_rol} autorId={post.usuario_id} />
        <div className="flex items-center gap-3 shrink-0">
          <Stars rating={post.rating} />
          <span className="text-xs text-gray-400"><TimeAgo dateStr={post.created_at} /></span>
          {/* Solo superadmin/moderador pueden borrar */}
          {(['superadmin', 'moderador'].includes(currentUser?.rol)) && (
...
export default function ComunidadPage() {
  ...
  // Aplicar búsqueda
  const filtered = (tab === 'resenas' ? resenas : recomendaciones).filter(p =>
    !query ||
    (p.titulo || '').toLowerCase().includes(query.toLowerCase()) ||
    (p.contenido || '').toLowerCase().includes(query.toLowerCase()) ||
    (p.autor_nombre || '').toLowerCase().includes(query.toLowerCase())
  );
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Buscar en el foro por título, contenido o autor..."
          className="w-full pl-10 pr-10 py-2.5 rounded-xl bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-sm text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500 transition-all"
        />
        {query && (
