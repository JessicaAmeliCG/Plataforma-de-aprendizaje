/**
 * AdminLayout.jsx — Layout para SuperAdmin
 * Hereda la estructura visual de CreatorLayout pero con menú propio
 */
...
import {
  ...
  Moon, Sun, GraduationCap, Menu, ShieldCheck,
} from 'lucide-react';
...
export default function AdminLayout() {
  ...
  const displayUser = user || { nombre: 'Super Admin', email: '', avatar_color: 'from-rose-500 to-purple-700' };

  const NAV_ITEMS = [
    { to: '/admin/dashboard',   label: 'Panel Admin',      Icon: LayoutDashboard },
    { to: '/admin/cursos',      label: 'Todos los Cursos', Icon: BookOpen },
    { to: '/admin/usuarios',    label: 'Usuarios',         Icon: Users },
    { to: '/admin/moderadores', label: 'Moderadores',      Icon: ShieldCheck },
    { to: '/admin/comunidad',   label: 'Comunidad',        Icon: MessageSquare },
    { to: '/admin/analiticas',  label: 'Analíticas',       Icon: BarChart2 },
    { to: '/admin/ajustes',     label: 'Ajustes',          Icon: Settings },
  ];

  const ROUTE_TITLES = {};
  NAV_ITEMS.forEach(i => { ROUTE_TITLES[i.to] = i.label; });
  const pageTitle = ROUTE_TITLES[location.pathname] ?? 'Administración';
      ...
      <aside className={`fixed lg:relative inset-y-0 left-0 z-50 flex flex-col border-r shadow-sm transition-all duration-300 ease-in-out bg-white/90 dark:bg-gray-950/90 backdrop-blur-xl ${
        ...
        <div className="flex items-center h-16 border-b border-gray-200 dark:border-gray-800 px-4 gap-3 overflow-hidden">
          <div className="relative shrink-0">
            ...
            <span className="absolute -top-1 -right-1 w-3 h-3 bg-rose-500 rounded-full border-2 border-white dark:border-gray-950" />
          </div>
          <div className={`transition-all duration-300 ${collapsed ? 'opacity-0 w-0' : 'opacity-100 w-auto'} overflow-hidden whitespace-nowrap`}>
            ...
            <span className="ml-2 text-xs font-bold text-white bg-rose-500 px-1.5 py-0.5 rounded-md">Admin</span>
          </div>
        </div>
        ...
        <div className="p-2 border-t border-gray-200 dark:border-gray-800 space-y-1">
          {!collapsed && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-rose-50 dark:bg-rose-900/20">
              <div className={`w-7 h-7 rounded-full shrink-0 bg-gradient-to-br ${displayUser.avatar_color || 'from-rose-500 to-purple-700'} flex items-center justify-center text-white font-bold text-xs`}>
              ...
              <div className="min-w-0">
                ...
                <p className="text-[10px] text-rose-500 font-bold">Super Admin</p>
              </div>
            </div>
          )}
          <button
        ...
        <button
          ...
          className="hidden lg:flex absolute -right-3 top-[72px] w-6 h-6 rounded-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm items-center justify-center text-gray-500 hover:text-rose-600 transition-all z-10"
        >
        ...
        <header className="flex items-center justify-between h-16 px-4 lg:px-6 shrink-0 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border-b shadow-sm z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-rose-500 font-semibold mt-0.5">Panel de Administración</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div
              ...
              className={`w-9 h-9 rounded-xl bg-gradient-to-br ${displayUser.avatar_color || 'from-rose-500 to-purple-700'} flex items-center justify-center text-white font-bold text-sm cursor-pointer ring-2 ring-rose-400/30 hover:ring-rose-400/60 transition-all`}
            >
              {getInitials(displayUser.nombre)}
            </div>
          </div>
